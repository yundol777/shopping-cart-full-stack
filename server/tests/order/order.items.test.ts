...
describe("POST /orders/items", () => {
  ...
  it("선택한 장바구니 상품이 존재하지 않으면 404를 반환한다.", async () => {
    const response = await request(app)
      .post("/orders/items")
      .send({ selectedCartItemIds: [999] });

    expect(response.status).toBe(404);
    expect(response.body.code).toBe("CART_ITEM_NOT_FOUND");
  });

  it("선택한 장바구니 상품 id 배열이 없으면 400을 반환한다.", async () => {
    const response = await request(app).post("/orders/items").send({});
